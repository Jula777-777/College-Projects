...
void GUI::displayGameState(const Player& player, const std::vector<Block>& blocks){
	...
	{
		blockSheetTexture->render(renderer, block.objectPart.position, nullptr);
	}


    //Render player
	playerSheetTexture->render(renderer, player.objectPart.position, playerClips[player.animation.currentSprite]);
...
{
	if (player.animation.currentSprite < (int)playerClips.size())
	{
		return { playerClips[player.animation.currentSprite]->w, playerClips[player.animation.currentSprite]->h };
	}
	return { 0,0 };
}
...
{	
	if (type == Object::ObjectType::block)
		return { 50,50 };
	else
		return { 0,0 };
}
