...
void GUI::displayGameState(const Engine& engine){
	auto [player, enemies, blocks] = engine.getObjects();
	//Clear screen
	...
	{
		objectTextures[block.getName()]->render(renderer, block.getPosition(), nullptr);
	}
	for (auto enemy : enemies)
	{
		objectTextures[Object::Type::enemy]->render(renderer, enemy.getPosition(), enemyClips[enemy.getCurrentSprite()]);
	}

    //Render player
	objectTextures[Object::Type::player]->render(renderer, player.getPosition(), playerClips[player.getCurrentSprite()]);
